...
RUN TEMP_DEB="$(mktemp)" && \
    wget -O "$TEMP_DEB" 'https://packages.ubiquityrobotics.com/ubuntu/ubiquity/pool/main/r/ros-kinetic-raspicam-node/ros-kinetic-raspicam-node_0.4.0-2xenial_armhf.deb' && \
    apt-get install "$TEMP_DEB" && \
    rm -f "$TEMP_DEB"
